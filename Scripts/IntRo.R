...
data <- read.csv(file = "Data/ecotox_survival.csv")
data2 <- read.csv(file = "Data/df.csv")
...
unique(data$Treatment) # gives all unique values
class(data$Treatment)  # tells you the class of the data
table(data$Treatment)  # build a table of counts at each factor  is.na(data)
# figure out if there are any NA's in the object
...
dotchart(data$PrecSurv)  # Dotchart of a column in data
...
library(ggplot2)
g <- ggplot(data = data, aes(x = Hour, y = PrecSurv)) +
  geom_point(aes(color = Treatment)) + ggtitle("Mortality by
  Treatment Group") + theme(plot.title = element_text(hjust = 0.5))

g <- g + geom_line(aes(group = TreatmentRep, color = Treatment,
  Linetype = Treatment), size=1, arrow = arrow(length = unit(0.1,
  "inches")))

g <- g + binomial_smooth(aes(group = Treatment,
  color = Treatment), formula = y ~ x)

g <- g + facet_grid(. ~ Treatment)

# Let's take a break from the R console and focus on what's going
# on under the hood of lm(); the function to do linear regression

# Linear Regression
# Try to run a lm() on the continuous data in the dataset data2

# ANOVA
# Try to run an ANOVA on the categorical data in the dataset data

# Your Data
# Import, clean, and run an appropriate analysis on your data set

# Your Data
# Import, clean, and run an appropriate analysis on your data set

# Here's the steps to getting something out of this data with lm()
# Remember to explore data first
head(data)
str(data)
# Plot it to check relationship
plot(data)
# Select you variables of interest - "Mortality" & "Treatment"
plot(data$Mortality, data$Treatment)
# Set up the linear model
AN.1 <- lm(Mortality ~ Treatment, data=data)
# Check assumptions!
par(mfrow=c(2,2)) # don't forget to do undo graphical params change
plot(AN.1)
# Check significance
summary(AN.1)
anova(AN.1)
